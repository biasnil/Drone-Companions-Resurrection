...
function DCO:new()
	...
	TweakDB:SetFlat("DCO.GigaBeam.hitCooldown", 0.12, 'Float')
	...
	TweakDB:SetFlat("DroneOctantActions.ShootDefault_inline6.numberOfShots", 10, 'Int32')
	...
	Override('ScriptedPuppet', 'ProcessLoot', function(self, wrappedMethod)
	  local charRecord = TweakDBInterface.GetCharacterRecord(self:GetRecordID())
	  if not charRecord or not charRecord:TagsContains(CName.new("Robot")) then
	    if wrappedMethod then wrappedMethod(self) end
	    return
	  end
	  ...
	  if not self:IsLooted() then
	    ...
	    if itemFlat then
	      ...
	      if itemRec then
	        ingdata = CraftingSystem.GetInstance():GetItemCraftingCost(itemRec)
	      end
	    end
	
	    local death_loot = Game.GetStatsSystem():GetStatValue(Game.GetPlayer():GetEntityID(), gamedataStatType.NPCUnequipItemDuration) > 0
	    ...
	    if self.CacheLootForDropping then
	      self:CacheLootForDropping()
	    end
	  end
	end)
	...
	local function ensureHomingGDM()
	  ...
	  local bases = {
	    'Items.GrenadeBiohazardHoming_inline0',
	    'Items.GrenadeFlashHoming_inline0',
	    'Items.GrenadeFlash_inline0'
	  }
	...
	if not recordExists('DCO.AndroidGrenadeFragHoming') then
	  createAndroidDamageGrenade('DCO.AndroidGrenadeFragHoming', recordExists('Items.GrenadeFragHoming') and 'Items.GrenadeFragHoming' or 'Items.GrenadeFragRegular', 8)
	end
	...
	if not recordExists('DCO.AndroidGrenadeBiohazardHoming') then
	  createAndroidSEGrenade('DCO.AndroidGrenadeBiohazardHoming', recordExists('Items.GrenadeBiohazardHoming') and 'Items.GrenadeBiohazardHoming' or 'Items.GrenadeBiohazardRegular', 1.5, 'BaseStats.ChemicalDamage', 'Attacks.LowChemicalDamageOverTime')
	end
	...
	if not recordExists('DCO.AndroidGrenadeEMPHoming') then
	  createAndroidSEGrenade('DCO.AndroidGrenadeEMPHoming', recordExists('Items.GrenadeEMPHoming') and 'Items.GrenadeEMPHoming' or 'Items.GrenadeEMPRegular', 1.5, 'BaseStats.ElectricDamage', 'Attacks.LowElectricDamageOverTime')
	end
	...
	if not recordExists('DCO.AndroidGrenadeIncendiaryHoming') then
	  createAndroidSEGrenade('DCO.AndroidGrenadeIncendiaryHoming', recordExists('Items.GrenadeIncendiaryHoming') and 'Items.GrenadeIncendiaryHoming' or 'Items.GrenadeIncendiaryRegular', 1.5, 'BaseStats.ThermalDamage', 'Attacks.EnemyNetrunnerThermalDamageOverTime')
	end
	...
	local freeflats = {'addAxisRotationDelay','addAxisRotationSpeedMax','addAxisRotationSpeedMin','effectCooldown','freezeDelayAfterBounce','minimumDistanceFromFloor','stopAttackDelay'}
	for _, v in ipairs(freeflats) do
	  local src = TweakDB:GetFlat('Items.GrenadeCuttingRegular.'..v)
	  if src ~= nil then
	    local t = type(src) == 'number' and (math.type and math.type(src) == 'integer' and 'Int32' or 'Float') or nil
	    if t then TweakDB:SetFlat('DCO.AndroidGrenadeCutting.'..v, src, t) else TweakDB:SetFlat('DCO.AndroidGrenadeCutting.'..v, src) end
	  end
	end
...
function makeSpreadEffectors(objectAction, count)
	...
	--TweakDB:SetFlat(objectAction.."Effect".."SpreadEffector.spreadDistance", 16, 'Int32')
	...
	for i=1,(count-1) do
		...
		--TweakDB:SetFlat(objectAction.."Effect"..i.."SpreadEffector.spreadDistance", 16, 'Int32')
	end
