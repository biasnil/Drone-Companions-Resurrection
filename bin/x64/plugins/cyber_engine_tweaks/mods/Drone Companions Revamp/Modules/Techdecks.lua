...
function DCO:new()
  ...
  -- Quickhacks list UI hints
  Override('QuickhacksListGameController', 'SelectData', function(self, data, wrappedMethod)
    if wrappedMethod then wrappedMethod(data) end
    if not data or not data.actionOwner then return end

    local ent = Game.FindEntityByID(data.actionOwner)
    ...
    local title = data.title or (self.selectedData and self.selectedData.title) or nil
    if not title then return end

    local txt = GetLocalizedText(title)
    if ent.GetNPCType and ent:GetNPCType() == gamedataNPCType.Mech and txt == GetLocalizedText("LocKey#266") then
      if self.warningText then inkTextRef.SetText(self.warningText, Mech_No_Repair_String) end
    end
    if txt == GetLocalizedText("LocKey#256") and Game.GetPlayer() and Game.GetPlayer():IsInCombat() then
      if self.warningText then inkTextRef.SetText(self.warningText, Shutdown_No_Combat_String) end
    end
    if txt == GetLocalizedText("LocKey#10547") and Game.GetPlayer() and Game.GetPlayer():IsInCombat() then
      if self.warningText then inkTextRef.SetText(self.warningText, Shutdown_No_Combat_String) end
    end
    if txt == GetLocalizedText("LocKey#3665") and not (ent.GetNPCType and ent:GetNPCType() == gamedataNPCType.Android) then
      if self.warningText then inkTextRef.SetText(self.warningText, Kerenzikov_Not_Android_String) end
    end
  end)

  -- Sandevistan-like dilation when Overdrive SE is applied (SE itself is defined in YAML)
  Observe('NPCPuppet', 'OnStatusEffectApplied', function(self, statusEffect)
    if TweakDBInterface.GetCharacterRecord(self:GetRecordID()):TagsContains(CName.new("Robot")) then
  ...
  -- Grant abilities (defined in YAML) to robots
  ...
  -- Android suicide ability (SE/ability itself YAML-owned); just attach it
  for _,v in ipairs(Android_List) do addToList(v..".abilities", "DCO.AndroidSuicideAbility") end

  -- Runtime-only dismember+kill for SuicideWithWeapon
  Observe('NPCPuppet', 'OnStatusEffectApplied', function(self, statusEffect)
  ...
  -- Block equipping DCO mods from backpack
  local function DCO_ShouldBlockBackpackClick(evt)
    ...
    local cat = rec:ItemCategory():Type()
    if cat == gamedataItemCategory.Consumable then return false end
    if rec:TagsContains(CName.new("Food"))   then return false end
    if rec:TagsContains(CName.new("Drink"))  then return false end
    if rec:TagsContains(CName.new("Alcohol")) then return false end
    if rec:TagsContains(CName.new("DCOMod")) and rec:TagsContains(CName.new("Fragment")) then return true end
    return false
  end
  Override('BackpackMainGameController', 'OnItemDisplayClick', function(self, evt, wm)
    if DCO_ShouldBlockBackpackClick(evt) then return end
    if wm then wm(evt) end
  end)
